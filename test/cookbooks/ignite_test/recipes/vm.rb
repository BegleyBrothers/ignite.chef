...
# chef-client run.
ignite_vm 'busybox_ls' do
  repo 'busybox'
  command 'ls -la /'
  not_if "[ ! -z `ignite ps -qaf 'name=busybox_ls$'` ]"
  action :run
end

# The :run_if_missing action will only run once. It is the default
# action.
ignite_vm 'alpine_ls' do
  repo 'alpine'
  tag '3.1'
  command 'ls -la /'
...
ignite_vm 'another_echo_server' do
  repo 'alpine'
  tag '3.1'
  command 'nc -ll -p 7 -e /bin/cat'
  port '7'
...
ignite_vm 'an_udp_echo_server' do
  repo 'alpine'
  tag '3.1'
...
ignite_vm 'unstarted_redeployer' do
  repo 'alpine'
  tag '3.1'
  command 'sh -c "trap exit 0 SIGTERM; while :; do sleep 1; done"'
  port '7'
...
execute 'redeploy redeployers' do
  ...
  notifies :redeploy, 'ignite_vm[redeployer]', :immediately
  notifies :redeploy, 'ignite_vm[unstarted_redeployer]', :immediately
  action :run
end
...
ignite_image 'chef_container' do
  tag 'latest'
  source '/chefbuilder'
...
ignite_vm 'ohai_again_debian' do
  repo 'debian'
  volumes_from 'chef_container'
  entrypoint '/opt/chef/embedded/bin/ohai'
  command 'platform'
...
ignite_image 'cmd_test' do
  tag 'latest'
  source '/cmd_test'
  action :build_if_missing
end

ignite_vm 'cmd_test' do
  action :run_if_missing
end
...
# Inspect volume container with test-kitchen bussers
ignite_vm 'attached_with_timeout' do
...
ignite_vm 'cap_add_net_admin' do
  repo 'debian'
...
ignite_vm 'cap_add_net_admin_error' do
  repo 'debian'
  command 'bash -c "ip addr add 10.9.8.7/24 brd + dev eth0 label eth0:0 ; ip addr list"'
  action :run_if_missing
end
...
ignite_vm 'cap_drop_mknod_error' do
  repo 'debian'
  command 'bash -c "mknod -m 444 /dev/urandom2 c 1 9 ; ls -la /dev/urandom2"'
  action :run_if_missing
end
...
ignite_vm 'reboot_survivor_retry' do
  repo 'alpine'
  tag '3.1'
  command 'nc -ll -p 123 -e /bin/cat'
  port '123'
...
ignite_vm 'link_target_1' do
  repo 'alpine'
  tag '3.1'
  env ['ASD=asd']
  command 'ping -c 1 hello'
  links 'link_source:hello'
  subscribes :run, 'ignite_vm[link_source]'
  action :run_if_missing
end

# docker logs linker_target_2
ignite_vm 'link_target_2' do
  repo 'alpine'
  tag '3.1'
  command 'env'
  links ['link_source:hello']
...
ignite_vm 'link_target_3' do
  ...
  subscribes :run, 'ignite_vm[link_source]'
  subscribes :run, 'ignite_vm[link_source_2]'
  action :run_if_missing
end

# docker logs linker_target_4
ignite_vm 'link_target_4' do
  repo 'alpine'
  tag '3.1'
  command 'env'
  links ['link_source:hello', 'link_source_2:hello_again']
...
execute 'redeploy_link_source' do
  ...
  notifies :redeploy, 'ignite_vm[link_source_2]'
  notifies :redeploy, 'ignite_vm[link_target_1]'
  notifies :redeploy, 'ignite_vm[link_target_2]'
  notifies :redeploy, 'ignite_vm[link_target_3]'
  notifies :redeploy, 'ignite_vm[link_target_4]'
  action :run
end
...
ignite_vm 'another_link_source' do
  repo 'alpine'
  tag '3.1'
...
ignite_vm 'another_link_target' do
  repo 'alpine'
  tag '3.1'
  command 'ping -c 1 hello'
...
ignite_vm 'dangler_volume_remover' do
  vm_name 'dangler'
  remove_volumes true
  action :delete
end
...
ignite_tag 'mutator_from_busybox' do
  target_repo 'busybox'
  target_tag 'latest'
...
execute 'commit mutator' do
  ...
  notifies :commit, 'ignite_vm[mutator]', :immediately
  notifies :export, 'ignite_vm[mutator]', :immediately
  notifies :redeploy, 'ignite_vm[mutator]', :immediately
  action :run
end
...
ignite_vm 'change_network_mode' do
  repo 'alpine'
  tag '3.1'
  command 'sh -c "trap exit 0 SIGTERM; while :; do sleep 1; done"'
  network_mode 'host'
...
file '/overrides/Dockerfile' do
  ...
  EXPOSE 4321
  EOF
  notifies :build, 'ignite_image[overrides]'
  action :create
end

ignite_image 'overrides' do
  tag 'latest'
  source '/overrides'
  force true
  action :build_if_missing
  notifies :redeploy, 'ignite_vm[overrides-1]'
  notifies :redeploy, 'ignite_vm[overrides-2]'
end

ignite_vm 'overrides-1' do
  repo 'overrides'
  action :run
end

ignite_vm 'overrides-2' do
  repo 'overrides'
  user 'operator'
  command 'sh -c "trap exit 0 SIGTERM; while :; do sleep 1; done"'
  env ['FOO=biz']
...
file '/kill_after/loop.sh' do
  ...
  while true; do :; done
  EOF
  notifies :build, 'ignite_image[kill_after]'
  action :create
end
...
ignite_vm 'cmd_change' do
  repo 'cmd_change'
  action :run
end

ignite_image 'cmd_change' do
  tag 'latest'
  action :build
  source '/usr/local/src/cmd_change_two'
  not_if { ::File.exist?('/marker_cmd_change') }
  notifies :redeploy, 'ignite_vm[cmd_change]'
end
