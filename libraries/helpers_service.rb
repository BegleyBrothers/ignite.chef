...
IPV4_CIDR ||= /(([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])\.){3}([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])(\/([0-9]|[1-2][0-9]|3[0-2]))/.freeze
    ...
    module Service
      ...
      def connect_host
        ...
        sorted = coerce_host(host).sort do |a, b|
          c_a = 1 if a =~ /^unix:/
          c_a = 2 if a =~ /^fd:/
          c_a = 3 if a =~ /^tcp:\/\/127.0.0.1:/
          c_a = 4 if a =~ /^tcp:\/\/(192\.168|10\.|172\.1[6789]\.|172\.2[0-9]\.|172\.3[01]\.).*:/

          c_a = 5 if a =~ /^tcp:\/\/0.0.0.0:/
          c_a ||= 6
          c_b = 1 if b =~ /^unix:/
          c_b = 2 if b =~ /^fd:/
          c_b = 3 if b =~ /^tcp:\/\/127.0.0.1:/
          c_b = 4 if b =~ /^tcp:\/\/(192\.168|10\.|172\.1[6789]\.|172\.2[0-9]\.|172\.3[01]\.).*:/

          c_b = 5 if b =~ /^tcp:\/\/0.0.0.0:/
          c_b ||= 6
          c_a <=> c_b
        end
        if sorted.first =~ /^tcp:\/\/0.0.0.0:/
          r = sorted.first.match(/(?<proto>.*):\/\/(?<socket>[^:]+):?(?<port>\d+)?/)
      ...
      def connect_socket
        ...
        return nil if host.grep(/unix:\/\/|fd:\/\//).empty?
