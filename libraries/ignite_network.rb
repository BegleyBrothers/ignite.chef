...
module IgniteCookbook
  class IgniteNetwork < IgniteBase
    ...
    property :vm, String, desired_state: false
    ...
    property :network, Ignite::Network, desired_state: false
    ...
    load_current_value do
      begin
        with_retries { network Ignite::Network.get(network_name, {}, connection) }
    ...
    action :create do
      converge_if_changed do
        ...
        with_retries do
          ...
          Ignite::Network.create(new_resource.network_name, options)
        end
      end
    end
    ...
    action :connect do
      unless new_resource.vm
        raise Chef::Exceptions::ValidationFailed, 'VM id or name is required for action :connect'
      end

      if current_resource
        vm_index = current_resource.network.info['Vms'].values.index { |c| c['Name'] == new_resource.vm }
        if vm_index.nil?
          converge_by("connect #{new_resource.vm}") do
            with_retries do
              current_resource.network.connect(new_resource.vm)
            end
          end
        end
      else
    ...
    action :disconnect do
      unless new_resource.vm
        raise Chef::Exceptions::ValidationFailed, 'VM id or name is required for action :disconnect'
      end

      if current_resource
        vm_index = current_resource.network.info['Vms'].values.index { |c| c['Name'] == new_resource.vm }
        unless vm_index.nil?
          converge_by("disconnect #{new_resource.vm}") do
            with_retries do
              current_resource.network.disconnect(new_resource.vm)
            end
          end
        end
      else
