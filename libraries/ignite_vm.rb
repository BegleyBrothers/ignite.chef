...
module IgniteCookbook
  class IgniteContainer < IgniteBase
    ...
    property :vm_name, String, name_property: true
    property :repo, String, default: lazy { vm_name }
    ...
    # Used to store the state of the Ignite vm
    property :vm, Ignite::Vm, desired_state: false

    # Used to store the state of the Ignite vm create options
    property :create_options, Hash, default: {}, desired_state: false

    # Used by :stop action. If the vm takes longer than this
    ...
    def coerce_links(v)
      case v
      when IgniteBase::UnorderedArray, nil
        v
      else
    ...
    def coerce_volumes(v)
      case v
      when IgniteBase::PartialHash, nil
        v
      when Hash
        IgniteBase::PartialHash[v]
      else
        b = []
        ...
        b = nil if b.empty?
        volumes_binds b
        return IgniteBase::PartialHash.new if v.empty?
        v.each_with_object(IgniteBase::PartialHash.new) { |volume, h| h[volume] = {} }
      end
    end

    def state
      # Always return the latest state, see #510
      Ignite::Vm.get(vm_name, {}, connection).info['State']
    ...
    def wait_running_state(v)
      ...
      raise Ignite::Error::TimeoutError, "Container #{vm_name} failed to change to #{desired_state_str} state after #{tries} seconds"
    end
    ...
    # for full specs of image parsing, see spec/helpers_vm_spec.rb
    #
    ...
    load_current_value do
      # Grab the vm and assign the vm property
      begin
        with_retries { vm Ignite::Vm.get(vm_name, {}, connection) }
      rescue Ignite::Error::NotFoundError
        current_value_does_not_exist!
      end

      # Go through everything in the vm and set corresponding properties:
      # c.info['Config']['ExposedPorts'] -> exposed_ports
      (vm.info['Config'].to_a + vm.info['HostConfig'].to_a).each do |key, value|
      ...
      # load vm specific labels (without engine/image ones)
      load_vm_labels

      # these are a special case for us because our names differ from theirs
      restart_policy vm.info['HostConfig']['RestartPolicy']['Name']
      restart_maximum_retry_count vm.info['HostConfig']['RestartPolicy']['MaximumRetryCount']
      volumes_binds vm.info['HostConfig']['Binds']
      ro_rootfs vm.info['HostConfig']['ReadonlyRootfs']
      ip_address ip_address_from_vm_networks(vm) unless ip_address_from_vm_networks(vm).nil?
    end

    # Gets the ip address from the existing vm
    # current docker api of 1.16 does not have ['NetworkSettings']['Networks']
    # For docker > 1.21 - use ['NetworkSettings']['Networks']
    #
    #   @param vm [Ignite::Vm] A vm object
    #   @returns [String] An ip_address
    def ip_address_from_vm_networks(vm)
      # We use the first value in 'Networks'
      # We can't assume it will be 'bridged'
      # It might also not match the new_resource value
      if vm.info['NetworkSettings'] &&
         vm.info['NetworkSettings']['Networks'] &&
         vm.info['NetworkSettings']['Networks'].values[0] &&
         vm.info['NetworkSettings']['Networks'].values[0]['IPAMConfig'] &&
         vm.info['NetworkSettings']['Networks'].values[0]['IPAMConfig']['IPv4Address']
    ...
    # Loads vm specific labels excluding those of engine or image.
    # This insures idempotency.
    def load_vm_labels
      image_labels = Ignite::Image.get(vm.info['Image'], {}, connection).info['Config']['Labels'] || {}
      engine_labels = Docker.info(connection)['Labels'] || {}

      labels = (vm.info['Config']['Labels'] || {}).reject do |key, val|
    ...
    action :create do
      ...
      converge_if_changed do
        ...
        with_retries do
          ...
          # Store the state of the options and create the vm
          new_resource.create_options = config
          Ignite::Vm.create(config, connection)
        end
      end
    end
    ...
    action :start do
      ...
      converge_by "starting #{new_resource.vm_name}" do
        with_retries do
          current_resource.vm.start

          unless new_resource.detach
            new_resource.timeout ? current_resource.vm.wait(new_resource.timeout) : current_resource.vm.wait
          end
        end
    ...
    action :kill do
      return unless state['Running']
      converge_by "killing #{new_resource.vm_name}" do
        with_retries { current_resource.vm.kill(signal: new_resource.signal) }
      end
    end
    ...
    action :pause do
      ...
      converge_by "pausing #{new_resource.vm_name}" do
        with_retries { current_resource.vm.pause }
      end
    end

    action :unpause do
      return if current_resource && !state['Paused']
      converge_by "unpausing #{new_resource.vm_name}" do
        with_retries { current_resource.vm.unpause }
      end
    end

    action :restart do
      kill_after_str = " (will kill after #{new_resource.kill_after}s)" if new_resource.kill_after != -1
      converge_by "restarting #{new_resource.vm_name} #{kill_after_str}" do
        current_resource ? current_resource.vm.restart('timeout' => new_resource.kill_after) : call_action(:run)
      end
    end

    action :reload do
      converge_by "reloading #{new_resource.vm_name}" do
        with_retries { current_resource.vm.kill(signal: 'SIGHUP') }
      end
    end
    ...
    action :delete do
      ...
      call_action(:unpause)
      call_action(:stop)
      converge_by "deleting #{new_resource.vm_name}" do
        with_retries { current_resource.vm.delete(force: new_resource.force, v: new_resource.remove_volumes) }
      end
    end
    ...
    action :commit do
      converge_by "committing #{new_resource.vm_name}" do
        with_retries do
          new_image = current_resource.vm.commit
    ...
    action :export do
      raise "Please set outfile property on #{new_resource.vm_name}" if new_resource.outfile.nil?
      converge_by "exporting #{new_resource.vm_name}" do
        with_retries do
          ::File.open(new_resource.outfile, 'w') { |f| current_resource.vm.export { |chunk| f.write(chunk) } }
        end
      end
    end

    declare_action_class.class_eval do
      def validate_vm_create
        ...
        if new_resource.network_mode == 'vm' &&
           (
             !(new_resource.hostname.nil? || new_resource.hostname.empty?) ||
          ...
          raise Chef::Exceptions::ValidationFailed, 'Cannot specify hostname, dns, dns_search, mac_address, extra_hosts, exposed_ports, port_bindings, publish_all_ports, port when network_mode is vm.'
        end
      end
